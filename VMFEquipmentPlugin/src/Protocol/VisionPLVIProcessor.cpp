...
VisionPlviProcessor::VisionPlviProcessor()
{
    ...
    // S107/F6 수신 → 1차 ACK(Measure) 또는 2차 결과(RequestResult)로 분기
    // 프로토콜 상 동일 F6이므로, 수신 순서 기반:
    //   첫 번째 F6 → OnMeasure (검사 시작 ACK)
    //   두 번째 F6 → OnRequestResult (결과)
    disp.RegisterHandler(VisionPLVIProtocol::PLVIResult,
        [this](int, int, std::vector<uint8_t>&& body, int)
        {
            // 1차 ACK 수신 전이면 OnMeasure, 이후면 OnRequestResult
            if (!HasReceived(Measure))
                this->OnMeasure(std::move(body));
            else
                this->OnRequestResult(std::move(body));
        });
}
...
// ================================================================
// [Measure] 검사 시작 요청 (REQ_MEASURE, S107/F5)
//
// 패킷 조립:
...
//   cData[2]   = "CTrayX,CTrayY" 형태 문자열 (PLVI::CTRAY_X, CTRAY_Y 조합)
//   cData[3]   = Device 유무 배열 ("0,99,99,0,..." 콤마 구분) (PLVI::DEVICE_INFO)
// ================================================================
bool VisionPlviProcessor::MeasureAsync(const VMF::StringMap& params)
{
    ...
    // cData[2] → C-Tray 크기 "X,Y" 형태 (CTRAY_INFO 대신 CTRAY_X, CTRAY_Y 조합)
    {
...
// ================================================================
// [RequestResult] 결과 요청 (REQ_MEASURE, S107/F5)
//
// 패킷 조립:
//   nDataID    = PLVI 요청 ID
//   cData      = 미사용
// ================================================================
bool VisionPlviProcessor::RequestResultAsync(const StringMap& params)
{
    ClearLatestData(RequestResult);
...
// ================================================================
// [SetInformation] 정보 설정 (미사용 - 필요시 확장)
// ================================================================
bool VisionPlviProcessor::SetInformationAsync(const StringMap&)
{
    return false;
}
...
// ================================================================
// [OnRequestResult] 2차 응답 수신 → PLVI 검사 결과 (S107/F6)
//
// 파싱:
//   nStatus    → PLVIResult::STATUS         ("0"=ERROR, "1"=SUCCESS)
...
//
// Task에서 사용:
//   auto& data = vp->GetLatestData(RequestResult);
//   data[PLVIResult::STATUS]          → "1"이면 수신 성공
//   data[PLVIResult::TOTAL]  → "0"=OK, "1"=NG
//   data[PLVIResult::POCKET_RESULT]   → "0,99,1,2,11,..." 파싱
// ================================================================
void VisionPlviProcessor::OnRequestResult(ByteArray body)
{
    if (body.size() < sizeof(CPacketBody_S107F6))
    {
        ClearLatestData(RequestResult); return;
    }
    CPacketBody_S107F6 pkt;
    std::memcpy(&pkt, body.data(), sizeof(pkt));
    ...
    SetLatestData(RequestResult, data);
    SetReceived(RequestResult, true);
}

void VisionPlviProcessor::OnSetInformation(ByteArray)
{
    // 미사용
}
