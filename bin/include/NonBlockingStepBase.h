...
namespace VMF
{
    ...
    /// </summary>
    class NonBlockingStepBase : public ITask
    {
    public:
        enum CommonStep
        {
        ...
        /// </summary>
        ~NonBlockingStepBase() override = default;
        ...
        TaskResult Execute(Context& ctx, IActuator* actuator) override
        {
            ...
            {
                ctx.SetLastError("NonBlockingStepBase: Stop requested");
                m_state_ = CS_ERROR;
                return TR_ERROR;
            }
            ...
            if (!m_initialized_)
            {
                ...
                {
                    ctx.SetLastError(std::string("NonBlockingStepBase: exception in OnInitialize: ") + ex.what());
                    m_state_ = CS_ERROR;
                    return TR_ERROR;
                }
                catch (...)
                {
                    ctx.SetLastError("NonBlockingStepBase: unknown exception in OnInitialize");
                    m_state_ = CS_ERROR;
                    return TR_ERROR;
                }
            }
            ...
            {
                ctx.SetLastError(std::string("NonBlockingStepBase: exception in OnPoll: ") + ex.what());
                m_state_ = CS_ERROR;
                return TR_ERROR;
            }
            catch (...)
            {
                ctx.SetLastError("NonBlockingStepBase: unknown exception in OnPoll");
                m_state_ = CS_ERROR;
                return TR_ERROR;
            }
        }
