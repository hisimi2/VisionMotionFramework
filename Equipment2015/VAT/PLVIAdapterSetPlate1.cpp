...
ActError PLVIAdapterSetPlate1::MoveToZ(const VisionPosition& target)
{
    ...
    int ret = m_parts->AxisZ.Move(target.pos[2]);
...
ActError PLVIAdapterSetPlate1::SetLightState(int camIndex, bool on, int lightIndex)
{
    ...
    {
        case 0:  // PLVI 카메라
            m_parts->LampPLVI.SetStatus(on);
            break;
        case 1:  // 측면 카메라
            m_parts->LampSide.SetStatus(on);
            break;
        default:
            return ActError::ActFail;
    }
...
ActError PLVIAdapterSetPlate1::GetLightState(int camIndex, bool& outOn, int lightIndex)
{
    ...
    {
        case 0:  // PLVI 카메라
            outOn = m_parts->LampPLVI.GetStatus();
            break;
        case 1:  // 측면 카메라
            outOn = m_parts->LampSide.GetStatus();
            break;
        default:
            return ActError::ActFail;
    }
